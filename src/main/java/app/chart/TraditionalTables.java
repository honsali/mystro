...
public final class TraditionalTables {
    ...
    public record TermRange(double startDegreeInclusive, double endDegreeExclusive, Planet ruler) {
    }
    ...
    public static TriplicityRulers triplicityRulers(ZodiacSign sign, Triplicity triplicity) {
        return switch (triplicity) {
            ...
            case MIXED_HELLENISTIC, UNKNOWN -> new TriplicityRulers(null, null, null);
        };
    }
    ...
    public static Planet termRuler(double longitude, Terms terms) {
        if (terms == Terms.NONE || terms == Terms.UNKNOWN) {
            return null;
        }
    ...
    public static List<TermRange> termRanges(ZodiacSign sign, Terms terms) {
        if (terms == Terms.NONE || terms == Terms.UNKNOWN) {
            return List.of();
        }
        TermBoundary[] boundaries = terms == Terms.PTOLEMAIC ? ptolemaicTerms(sign) : egyptianTerms(sign);
        List<TermRange> ranges = new java.util.ArrayList<>();
        double start = 0.0;
        for (TermBoundary boundary : boundaries) {
            ranges.add(new TermRange(start, boundary.upperDegreeExclusive(), boundary.ruler()));
            start = boundary.upperDegreeExclusive();
        }
        return List.copyOf(ranges);
    }

    public static int planetJoyHouse(Planet planet) {
        return switch (planet) {
            case MERCURY -> 1;
            case MOON -> 3;
            case VENUS -> 5;
    ...
    private static TriplicityRulers ptolemaicTriplicityRulers(ZodiacSign sign) {
        return switch (element(sign)) {
            ...
            // Ptolemy, Tetrabiblos I.18: water is left to Mars, with Venus by day and
            // Moon by night as co-rulers. The common Mars rulership is represented in
            // Mystro's auxiliary/participating slot.
            case WATER -> new TriplicityRulers(Planet.VENUS, Planet.MOON, Planet.MARS);
        };
    }
