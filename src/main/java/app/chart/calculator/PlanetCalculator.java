...
public class PlanetCalculator implements Calculator {
    ...
    public void calculate(NatalChart natalChart, CalculationContext ctx) {
        ...
        // Mystro currently uses mean lunar nodes as a fixed calculation convention, not a selectable
        // doctrine option.
        ...
        PlanetPosition southNodePlanet = new PlanetPosition(Planet.SOUTH_NODE, southNodeLongitude, AstroMath.signOf(southNodeLongitude), AstroMath.degreeInSign(southNodeLongitude), southNodeLatitude, AstroMath.normalize(northNode.getRightAscension() + 180.0), -northNode.getDeclination(),
    ...
    private PlanetPosition calculatePlanet(Planet planet, int swissPlanetId, double julianDay, double ascendant, double sunLongitude, CalculationContext ctx) {
        ...
        validatePlanetValues(planet, values, error, ctx);
    ...
    private EquatorialPosition equatorialPosition(Planet planet, int swissPlanetId, double julianDay, CalculationContext ctx) {
        ...
        if (!Double.isFinite(values[0]) || !Double.isFinite(values[1])) {
            LOG.error("subject={} Swiss Ephemeris returned invalid equatorial values for {}: {}", ctx.getSubject().getId(), planet, error);
    ...
    private void validatePlanetValues(Planet planet, double[] values, StringBuilder error, CalculationContext ctx) {
        if (!Double.isFinite(values[0]) || !Double.isFinite(values[1]) || !Double.isFinite(values[2]) || !Double.isFinite(values[3])) {
            LOG.error("subject={} Swiss Ephemeris returned invalid values for {}: {}", ctx.getSubject().getId(), planet, error);
            throw new IllegalArgumentException("Calculation failed. See application logs.");
        }
    }
