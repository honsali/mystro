...
public class PlanetCalculator extends BaseCalculator {
    ...
    protected void executeCalculation() {
        ...
        addPlanet(planets, Planet.SUN, SweConst.SE_SUN, julianDay, ascendant, sunLongitude);
        addPlanet(planets, Planet.MOON, SweConst.SE_MOON, julianDay, ascendant, sunLongitude);
        addPlanet(planets, Planet.MERCURY, SweConst.SE_MERCURY, julianDay, ascendant, sunLongitude);
        addPlanet(planets, Planet.VENUS, SweConst.SE_VENUS, julianDay, ascendant, sunLongitude);
        addPlanet(planets, Planet.MARS, SweConst.SE_MARS, julianDay, ascendant, sunLongitude);
        addPlanet(planets, Planet.JUPITER, SweConst.SE_JUPITER, julianDay, ascendant, sunLongitude);
        addPlanet(planets, Planet.SATURN, SweConst.SE_SATURN, julianDay, ascendant, sunLongitude);

        PlanetPosition northNode = calculatePlanet(Planet.NORTH_NODE, SweConst.SE_MEAN_NODE, julianDay, ascendant, sunLongitude);
        if (northNode != null) {
            ...
            PlanetPosition southNodePlanet = new PlanetPosition(Planet.SOUTH_NODE, ctx.round(southNodeLongitude), ctx.signOf(southNodeLongitude), ctx.round(ctx.degreeInSign(southNodeLongitude)), ctx.round(-northNode.getLatitude()), ctx.round(-northNode.getDeclination()),
                    ...
                    ctx.termRuler(southNodeLongitude, ctx.getTerms()), ctx.round(angularDistance(southNodeLongitude, sunLongitude)), ctx.round(ctx.antiscia(southNodeLongitude)), ctx.round(ctx.contraAntiscia(southNodeLongitude)));
            planets.add(southNodePlanet);
        }
        basicChart.setPlanets(planets);
    }

    private void addPlanet(List<PlanetPosition> planets, Planet planet, int swissPlanetId, double julianDay, double ascendant, double sunLongitude) {
        PlanetPosition position = calculatePlanet(planet, swissPlanetId, julianDay, ascendant, sunLongitude);
        if (position != null) {
            planets.add(position);
        }
    }

    private PlanetPosition calculatePlanet(Planet planet, int swissPlanetId, double julianDay, double ascendant, double sunLongitude) {
        ...
        return new PlanetPosition(planet, ctx.round(longitude), ctx.signOf(longitude), ctx.round(ctx.degreeInSign(longitude)), ctx.round(values[1]), ctx.round(declination(swissPlanetId, julianDay)), ctx.round(values[3]), ctx.round(meanDailySpeed(planet)),
                ctx.round(values[3] / meanDailySpeed(planet)), values[3] < 0, ctx.houseOf(longitude, ascendant), angularity(ctx.houseOf(longitude, ascendant)), ctx.termRuler(longitude, ctx.getTerms()), ctx.round(angularDistance(longitude, sunLongitude)), ctx.round(ctx.antiscia(longitude)),
                ctx.round(ctx.contraAntiscia(longitude)));
    }

    private double declination(int swissPlanetId, double julianDay) {
        double[] values = new double[6];
        StringBuilder error = new StringBuilder();
